...
bool BufferPoolManager::SelectFrameImpl(frame_id_t &free_frame_id) {
  // no lock, for when we call this function, we already have the lock
  if (free_list_.size() > 0) {
    ...
    if (!replacer_->Victim(&free_frame_id)) {
      ...
      if (pages_[free_frame_id].is_dirty_) {
        disk_manager_->WritePage(pages_[free_frame_id].page_id_, pages_[free_frame_id].GetData());
...
bool BufferPoolManager::UnpinPageImpl(page_id_t page_id, bool is_dirty) {
  ...
  if (tar->GetPinCount() <= 0) {
    ...
    tar->pin_count_--;
    //作为最后一个unpin page的，需要将dirty page写入到disk？
    //作为Victim离开时，会强制flush page。
    if (is_dirty) {
...
// attention: flush to disk means force the page written to disk
